...
class ForLoop:
    ...
    place_holder = "INSIDE_LOOP"
    
    def __init__(self, cluster: Cluster, act_fun: bool, first_layer=False, signs=True):
        self.cluster = cluster
        self.loop_pattern = None
        self.fixed_index = None
        self.num_iterators = None
        self.iterator_symbols = None
        self.assignment_string = None
        self.function_string = None
        self.loop_string = None
    ...
    def find_cluster_structure(self):
        def stand_alone(cluster:Cluster):
            all_indices = Preprocessing.get_all_elements(cluster.layer_table_indices)
            if len(all_indices)==1:
                return True
            return False

        def one_dim_sum(cluster: Cluster):
        ...
        def upper_matrix_sum(cluster: Cluster):
            ...
            new_indices = [[None for _ in range(sz)] for _ in range(sz)]
            sgn = 1
        ...
        def lower_matrix_sum(cluster: Cluster):
            if any([i==1 for i in cluster.dims]):
                return False
            for i in range(cluster.dims[0]):
        ...
        def matrix_sum(cluster: Cluster):
            if any([i==1 for i in cluster.dims]):
                return False
            unique_indices = [list(x) for x in set(
                tuple(x) for x in cluster.layer_table_indices)]
            if len(unique_indices) == len(cluster.layer_table_indices):
                return True
            return False

        def row_sum(cluster: Cluster):
        ...
        def global_for_loop(cluster: Cluster):
            if any([isinstance(st, tuple) for st in cluster.starts]) and len(Preprocessing.get_all_elements(cluster.layer_table_indices))>1:
                return True
            return False
        
        def get_global_string(input, is_start=False):
            symbol = input[0]
            pattern = input[1]
            range_string = ''
            if symbol == 'i':
        ...
        if global_for_loop(self.cluster) == True:
            ...
            for st in self.cluster.starts:
                if st[0] != 's' and st[0] is not None:
            ...
            if self.num_iterators==0:
                ...
                self.assignment_string = self.cluster.layer_symbol + "[{}]".format(iterator_string)
            self.loop_string = "\t"
            for i,s_i in enumerate([j for j,st in enumerate(self.cluster.starts) if not (st[0] is None or st[0] == 's')]):
                self.loop_string += "for {} in range(".format(self.iterator_symbols[i])
                ...
                if len(self.cluster.starts[s_i])>2 and self.cluster.starts[s_i][2]=='symmetric_zero':
                    self.loop_string += "\t"*(i+2)
                    self.loop_string += "if {} == {}:\r\n{}continue\r\n".format(self.iterator_symbols[i-1], self.iterator_symbols[i], "\t"*(i+3))
            ...
            self.assignment_string = self.cluster.layer_symbol + \
                "[{}]".format(self.iterator_symbols[0])
            range_end = [i for i in self.cluster.dims if i>1][0]
            self.loop_string = ("\tfor {iterator1} in range({end1}):\r\n\t\t" + self.place_holder + "\r\n").format(
                iterator1=self.iterator_symbols[0], end1=range_end)
            ...
            self.iterator_symbols = self.iterator_symbols[:self.num_iterators]
            self.assignment_string = self.cluster.layer_symbol + \
                "[{},{}]".format(self.iterator_symbols[0], self.iterator_symbols[1])
            self.loop_string = ("\tfor {iterator1} in range({end1}):\r\n\t\tfor {iterator2} in range({end2}):\r\n\t\t\t"
                + "if {iterator1}=={iterator2}:\r\n\t\t\t\tcontinue\r\n\t\t\t" + self.place_holder + "\r\n").format(
                iterator1=self.iterator_symbols[0], end1=self.cluster.dims[0],
                iterator2=self.iterator_symbols[1], end2=self.cluster.dims[1])
            ...
            self.assignment_string = self.cluster.layer_symbol + \
                "[{},{}]".format(self.iterator_symbols[0], self.iterator_symbols[1])
            self.loop_string = ("\tfor {iterator1} in range({end1}):\r\n\t\tfor {iterator2} in range(0, {iterator1}-1):\r\n\t\t\t"
                + self.place_holder + "\r\n").format(iterator1=self.iterator_symbols[0], end1=self.cluster.dims[0],
                iterator2=self.iterator_symbols[1], end2=self.cluster.dims[1])
            ...
            self.assignment_string = self.cluster.layer_symbol + \
                "[{},{}]".format(self.iterator_symbols[0], self.iterator_symbols[1])
            self.loop_string = ("\tfor {iterator1} in range({end1}):\r\n\t\tfor {iterator2} in range({end2}):\r\n\t\t\t" + self.place_holder + "\r\n").format(
                iterator1=self.iterator_symbols[0], end1=self.cluster.dims[0], iterator2=self.iterator_symbols[1], end2=self.cluster.dims[1])
            ...
            self.iterator_symbols = self.iterator_symbols[:self.num_iterators]
            self.assignment_string = self.cluster.layer_symbol + \
                "[{},{}]".format(self.fixed_index, self.iterator_symbols[0])
            self.loop_string = ("\tfor {iterator} in range({end}):\r\n\t\t" + self.place_holder + "\r\n").format(
                iterator=self.iterator_symbols[0], end=self.cluster.dims[1])
            ...
            self.iterator_symbols = self.iterator_symbols[:self.num_iterators]
            self.assignment_string = self.cluster.layer_symbol + \
                "[{},{}]".format(self.iterator_symbols[0], self.fixed_index)
            self.loop_string = ("\tfor {iterator} in range({end}):\r\n\t\t" + self.place_holder + "\r\n").format(
                iterator=self.iterator_symbols[0], end=self.cluster.dims[0])

        if self.loop_pattern is None:
            raise ValueError("Cluster {} has no spatially identifiable loop pattern".format(
                self.cluster.cluster_index))

    def set_function_string(self):
        def get_global_op(cluster: Cluster, num_tabs: int):
            weights = []
            num_el = []
            bias = cluster.bias
            tabs = ""
            ...
            if len(weights)==1:
                ...
                if not cases or (bounds[0][1]-bounds[0][0])==(bounds[1][1]-bounds[1][0]):
                    ...
                    for case in cases:
                        ...
                        started = True
                        fn += ("{}if {} == {}:\r\n\t\t"+tabs).format(
                            add_el, altfns[case[0]], case[1])
                        
                        if abs(case[2])!=num_el[1-case[0]]-1:
                            if_statement = "if {} GSIGN {}".format(altfns[1-case[0]], case[2])
                            assign_statement = (":\r\n\t\t\t"+tabs+"{} = ASSIGN\r\n\t"+tabs).format(self.assignment_string)
                            sgns = [">", "<"]
                        elif case[2]>0 and kw_pairs[1-case[0]][0]!="offcol_sum" and kw_pairs[1-case[0]][0]!="offrow_sum":
                            altf = (fns[1-case[0]].replace("np.sum(",""))[:-1]
                            if_statement = "ifGSIGN np.all({}==1)".format(altf)
                            assign_statement = (":\r\n\t\t\t"+tabs+"{} = ASSIGN\r\n\t"+tabs).format(self.assignment_string)
                            sgns = [""," not"]
                        elif case[2]<0 and kw_pairs[1-case[0]][0]!="offcol_sum" and kw_pairs[1-case[0]][0]!="offrow_sum":
                            altf = (fns[1-case[0]].replace("np.sum(",""))[:-1]
        ...
        def check_means(w, b, lims, fns, altfns):
            ...
            if mn>1:
                ...
                if all([abs(t-round(t))<1e-4 for t in temp_weights]):
                    ...
                    for i,f in enumerate(fns):
                        fns[i] = f.replace('sum','mean')
                    for i,f in enumerate(altfns):
                        altfns[i] = f.replace('sum','mean')
            return w, b, lims, fns, altfns

        def check_all(w, b, lims, fns, tabs):
            n = len(w)
            for i in range(2**n):
                ...
                for j in range(n):
                    ...
                    fns[j] = fns[j].replace(')','')
                    fn += "np.all({}=={})".format(fns[j],sgns[j])
                fn += ":\r\n\t\t"+tabs
                fn += ("{} = {}\r\n\t"+tabs).format(self.assignment_string, int(all_sum))
                fn += ("else:\r\n\t\t" + tabs + "{} = {}\r\n").format(self.assignment_string, int(off_sums[0]))
                return fn
        ...
        def check_any(w, b, lims, fns, tabs):
            ...
            for s,sgn in enumerate(sgns):
                ...
                if sgn1*sgn2<0:
                    ...
                    fns[0] = fns[0].replace(')','')
                    fn = "if "
                    fn += "np.any({}!={})".format(fns[0],sgn)
                    fn += ":\r\n\t\t"+tabs
                    fn += ("{} = {}\r\n\t"+tabs).format(self.assignment_string, sgn2)
                    fn += ("else:\r\n\t\t" + tabs + "{} = {}\r\n").format(self.assignment_string, sgn1)
                    return fn
        ...
        def get_simpler_if_statements(w, b, fns, tabs):
            ...
            compare_string = "if {}{} GLSIGN ".format(w0_string, fn0)
            for i in range(len(w)):
                compare_string += w_strings[i]
                compare_string += fns[i]
            compare_string += ":\r\n\t\t" + tabs + self.assignment_string + " = ASSIGN\r\n\t" + tabs
        ...
        def get_mult_cases(w, b, num_el):
            ...
            labels = np.sign(sums)
            if any(labels==0):
                label_sets = [np.sign(labels-.5), np.sign(labels+.5)]
            else:
                label_sets = [labels]
        ...
        def get_op(kw_pair: list, id: int, do_abs=False):
            result = ""
            ...
            for i,f in enumerate(self.cluster.function_ids):
                if f==id:
                    ...
                    elif self.cluster.starts[i][0] is None:
                        op_iterators.append(str(self.cluster.starts[i][1]))            
            
            if key == "full_sum" or key == "mat_sum":
                res = w_str + "np.sum({input_symbol})".format(input_symbol=in_sym)
            elif key == "value":
                if (not isinstance(kw_pair[2], (list, tuple))) or len(kw_pair[2])==1:
                    res = w_str + "{input_symbol}[{iterator}]".format(
                        input_symbol=in_sym, iterator=op_iterators[0])
                else:
                    res = w_str + "{input_symbol}[{iterator1}, {iterator2}]".format(
                        input_symbol=in_sym, iterator1=op_iterators[0], iterator2=op_iterators[1])
            elif key == "part_sum":
                sum_string = ''
                for i,s in enumerate(kw_pair[2]):
                    ...
                    if isinstance(s,int) or len(s)==1:
                        sum_string += "{}[{}]".format(in_sym,s)
                    else:
                        sum_string += "{}[{},{}]".format(in_sym,s[0],s[1])
                res = w_str + "({})".format(sum_string)
                ...
                raise ValueError(
                    "The given key value, {}, is not a valid key".format(key))
            result = res
            return result
        ...
        if fn is None:
            ...
            if self.cluster.bias > 0:
                ...
                fn += " - " + str(-int(self.cluster.bias))
            self.function_string = "np.sign({})".format(fn)
        else:
            self.function_string = fn
            self.global_found = True

    def write_code(self, filename, initialization):
