from distill import condense
import numpy as np

def main():
    problem_name = 'maxsat'
    ENN_layers_filenames = ['/data/sat/ENN/maxsat_2359800_1.csv',
    ...
    print("\nTesting generalized distilled code on random Boolean formulae")
...
def test_code(distill_func, input_dims):
    ...
    for nc in num_clauses:
        print('{} vars, {} clasuses -   '.format(variables, nc), end='')
        total_sat = [0, 0, 0]
        ...
        for alg_ind, alg in enumerate(algs):
            ...
            print('{}: {:.2f}'.format(alg, mean_sat), end=' ')
        print()

def get_approx(clauses, num_sat=0, unsat=0):
